...
import { get_hint, HintLevel, assert_never, type LooseCardPlay } from "../hints/hints";
...
// execute_complex_hint and its helpers live in hints/execute_complex.ts
// so the test harness can import them without pulling console_player's
// env-dependent top-level code.
...
async function auto_play_turn(
    ...
    while (!done) {
        ...
        switch (hint.level) {
            ...
            // NO_MOVES and REARRANGE_PLAY are handled by the early-return
            // block above; TS narrows them away before the switch.
            default:
                assert_never(hint);
        }
        if (done) break;
    ...
    if (remaining.length > 0 && moves_played === 0) {
        record_got_stuck();
        const current_clean = state.board.every(s => {
            const st = json_to_card_stacks([s])[0].stack_type;
            return st !== CardStackType.INCOMPLETE &&
                   st !== CardStackType.BOGUS &&
                   st !== CardStackType.DUP;
        });
        const puzzle_board = current_clean && state.board.length > 0
            ? state.board
            : state.last_clean_board;

        // Skip trivial puzzles: small hand (<3) on a small board (<=8 stacks).
        const is_trivial = remaining.length < 3 && puzzle_board.length <= 8;
        if (!is_trivial) {
            save_puzzle(puzzle_board, remaining, player_index);
        }
    }
